...
var Lightbox = function () {
  ...
  var CTX = this,
    ...
    btns = '',
    template = `<div class="jslghtbx-contentwrapper" id="jslghtbx-contentwrapper">
                    <div class="img-zoom-box" id="img-zoom-box" mag-thumb="drag"></div>
                    <div class="btn-box" id="btn-box" style="position: absolute; left: -60px; bottom: 0px; background: #fff; width: 100%; padding: 3px 60px; text-align: right;"></div>
                </div>`,
  ...
  function getByGroup(group) {
    ...
    if (!CTX.opt.asyn) {
       ...
       // 异步处理
       arr = document.querySelectorAll('[data-jslghtbx-group=' + group + ']');
       console.log('异步取数据', arr);
    }
  ...
  function setOpt(opt) {
    ...
    CTX.opt = {
      ...
      asyn: opt.asyn || false,
      changeSize: opt.changeSize || false,
      // callbacks
    ...
    CTX.wrapper = document.getElementById('img-zoom-box');
    CTX.btnBox = document.getElementById('btn-box');
    console.log('btns', CTX.opt.btns);
    for (var i = 0; i < CTX.opt.btns.length; i++) {
        var $btn = $(CTX.opt.btns[i]);
        $btn.addClass('jslghtbx-btn');
        $(CTX.btnBox).append($btn);
    }
    $(CTX.btnBox).find('.jslghtbx-btn').on('click', function() {
        var index = $(this).index();
        console.log('btn click', index, $(this).index(), CTX.opt.funs[index]);
        CTX.opt.funs[index] && (CTX.opt.funs[index])(currImage.img.src);
    });

    addEvent(CTX.wrapper, 'click', function(e) {
        e.stopPropagation();
    })
    addEvent(CTX.btnBox, 'click', function(e) {
        e.stopPropagation();
    })
  ...
  function openBox(el, group, cb, event) {
    ...
    addClass(currImage.img, 'jslghtbx-img');
    

    // set el as current thumbnail
    currThumbnail = el;
    console.log('currImages', currImages, currImages.length > 0);
    ...
    if (CTX.opt.controls && currImages.length > 1) {
      initControls();
      repositionControls();
        var preBtn = document.getElementsByClassName('jslghtbx-prev')[0];
        var nextBtn = document.getElementsByClassName('jslghtbx-next')[0];
        var pos = getPos(currThumbnail, currGroup) + 1;
        removeClass(preBtn, 'disabled');
        removeClass(nextBtn, 'disabled');
        if (pos === 1) {
            addClass(preBtn, 'disabled');
        } else if (pos === currImages.length) {
    ...
    currImage.img.onload = function () {
      ...
      console.log('box height', CTX.wrapper.style.height);
      ...
      var checkClassInt = setInterval(function () {
        if (hasClass(CTX.box, 'jslghtbx-active')) {
          ...
          $(CTX.btnBox).css({'visibility': 'visible'});
          // set animation
          if (typeof CTX.opt.animation === 'number') {
  ...
  // 新增
  // 适用于异步加载
  CTX.openGroup = function (el, group) {
    if (el && group) {
        openBox(el, group, false, false, 'next', true)
    } else {
        console.info('参数不完整');
        return;
    }
  ...
  CTX.resize = function () {
    ...
    if (CTX.opt.dimensions && newImgHeight > currImage.originalHeight ||
      ...
      console.log('currImage', currImage.originalHeight, currImage.originalWidth);
    }
    currImage.img.setAttribute('width', newImgWidth);
    currImage.img.setAttribute('height', newImgHeight);
    currImage.img.setAttribute('style', 'margin-top: '  + (getHeight() - 120 - newImgHeight) / 2 +'px');
    console.log('CTX.box', CTX.box, newImgWidth, newImgHeight);
    CTX.wrapper.setAttribute('style', 'margin-top: 60px;height: ' + (getHeight() - 120) + 'px');
  ...
  CTX.close = function () {
    ...
    $(CTX.btnBox).css({'visibility': 'hidden'});
