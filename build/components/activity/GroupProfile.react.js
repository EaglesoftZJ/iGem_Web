...
var GroupProfile = function (_Component) {
  ...
  GroupProfile.calculateState = function calculateState(prevState, nextProps) {
    ...
    return {
      ...
      message: _OnlineStore2.default.getMessage().replace('members', '位成员').replace('online', '在线').replace('offline', '离线')
    };
  };
  ...
  GroupProfile.prototype.renderToken = function renderToken() {
    ...
    var adminId = -1;
    adminId = _Linq2.default.from(group.members).firstOrDefault('$.isAdmin === true', { peerInfo: { peer: { id: -1 } } }).peerInfo.peer.id;
