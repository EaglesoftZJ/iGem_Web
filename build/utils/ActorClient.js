...
var ActorClient = function () {
  ...
  ActorClient.prototype.requestNickName = function requestNickName(nickName) {
    return new Promise(function (resolve, reject) {
  ...
  ActorClient.prototype.sendPassword = function sendPassword(password) {
    return new Promise(function (resolve, reject) {
  ...
  ActorClient.prototype.postWebservice = function postWebservice(config) {
    var xhr = false;
    //请求体
    if (window.XMLHttpRequest) {
      ...
      try {
        ...
        console.log('xhr error:不支持xmlhttprequest请求');
      }
    }
    //打开连接
  ...
  ActorClient.prototype.postOAWebservice = function postOAWebservice(config) {
    var xhr = false;
    //请求体
    if (window.XMLHttpRequest) {
      xhr = new XMLHttpRequest();
    ...
    if (status >= 200 && status < 300) {
      var res = JSON.parse(xhr.responseText.replace('<?xml version="1.0" encoding="utf-8"?>', '').replace('<string xmlns="http://www.eaglesoft.cn">', '').replace('</string>', '').trim());
      config.success && config.success(res);
    } else {
      return config.fail && config.fail(status);
    }
